...
const unsigned char* make_header(const unsigned int height, const unsigned int width)
{
    ...
    header.bfSize = sizeof(header) + static_cast<unsigned long long>(include_padding(height, width));
...
inline void SaveImage(const Texture3D& Texture, const char* FileName = "output.bmp")
{
    ...
    const int PaddingSize = (4 - (Width * (PIXELDEPTH / 8)) % 4) % 4;
	...
	for (size_t i = 0; i < Height; i++)
	{
		...
		{
            const Vector3 Pixel = Texture.Get(j, i).Clamp(0.0, 1.0);
            auto R = static_cast<unsigned char>(Pixel.X * 255);
            auto G = static_cast<unsigned char>(Pixel.Y * 255);
            auto B = static_cast<unsigned char>(Pixel.Z * 255);
